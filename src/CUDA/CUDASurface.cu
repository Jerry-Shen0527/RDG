#include "RDG/CUDA/CUDASurface.cuh"
...
void BlitLinearBufferToSurface(
    float4* buffer,
    nvrhi::cudaSurfaceObject_t surface_object,
    int width,
    int height)
{
    GPUParallelFor2D(
...
{
    GPUParallelFor(
        "Compose 4 channels into float4",
        size,
        GPU_LAMBDA_Ex(int i)
        {
...
{
    GPUParallelFor(
        "Compose 3 channels into float4",
        size,
        GPU_LAMBDA_Ex(int i)
        {
...
{
    GPUParallelFor(
        "Compose 4 channels into float3",
        size,
        GPU_LAMBDA_Ex(int i)
        {
